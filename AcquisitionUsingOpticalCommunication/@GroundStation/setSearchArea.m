...
function [gs,time,directionAccuracy_ut] = setSearchArea(time,gs,SSD,scEstByGsSeqP,error)
    ...
    gs.searchArea     = (posError/(SSD.AU*10)) * 3 ; %10AUくらいを想定．3sigmaをカバーする. 最低限200km*3くらいはカバーする
    gs.searchStep     = min(0.66e-6, ceil(gs.searchArea/20*1e8)*1e-8 ); %探索時の1stepあたりの間隔(rad). 0.1urad~0.8uradくらいで探索する
