...
function observationUpdateByScESIRF(obj,scTrue,earth, gsTrue,constant,type,time)
    % 何度目の観測か
...
%        Q = chol(Rm);
%        dy = inv(Q) * y;
%        matrixBeforeUpdate = [   inv(L), zeros(7,1);
%                              inv(Q)*Hm,      dy];
%        [~,matrixAfterUpdate] = qr(matrixBeforeUpdate);
%        
%        L = inv(matrixAfterUpdate(1:7,1:7));
%        dx = matrixAfterUpdate(1:7,8);
%        P = L.' * L;
%        X = X_star + L * dx;
   end
