...
function observationUpdateByScUkf(obj,scTrue,earth, gsTrue,constant,type,ukf,time)
    ...
    for i = 1:size(x_sp,2)
        ...
        if type == 1
            ...
            %ベクトル化
            [~,y_spM(:,i),~,~] = Spacecraft.alignReqInfo4Est(Y,y_sp(i),[],obj.R,"1u","ukf",obj.useObs);
        else
            y_sp(i) = Spacecraft.calcG_ur(x_spi,xve_ut,xvg_ut,xve_dr,xvg_dr,...
                                        dtAtGs, dt2w,constant,time,"2way");
            % ベクトル化
            [~,y_spM(:,i),~,~] = Spacecraft.alignReqInfo4Est(Y,y_sp(i),[],obj.R,"2u","ukf",obj.useObs);
        end
        if i ==1
            wm = ukf.w0_m;
        else
    ...
    if type == 1
        [Yv,y_meanV,~,Rm] = Spacecraft.alignReqInfo4Est(Y,y_mean,[],obj.R,"1u","ukf",obj.useObs);
    else
        [Yv,y_meanV,~,Rm] = Spacecraft.alignReqInfo4Est(Y,y_mean,[],obj.R,"2u","ukf",obj.useObs);
    end
