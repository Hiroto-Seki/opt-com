...
function [gsTrue,earth,scTrue,scEstByScEkf,scEstByGsEkf,time] = optComAcquisition_ESIRF(constant,time,error,gs,sc,gsTrue,earth,scTrue,scEstByScEkf,scEstByGsEkf,ekf,SSD)
...
for i = 1:length(time.list)-1
    ...
    if gsTrue.ut_counter == scTrue.ur_counter
        [scEstByScEkf.X, scEstByScEkf.P] = Spacecraft.timeUpdateESIRF(scEstByScEkf.X, scEstByScEkf.P, constant, time.simDt,time.simDt,error);   
    elseif time.list(i+1) < scTrue.t_ur(scTrue.ur_counter + 1) % 次の時刻まで観測がなかった場合
        ...
        % 観測までは推定値と共分散を時間伝搬
        [scEstByScEkf.X, scEstByScEkf.P] = Spacecraft.timeUpdateESIRF(scEstByScEkf.X, scEstByScEkf.P, constant, time.scDt1,time.simDt,error);
        % 観測量の計算
        ...
        scEstByScEkf.observationUpdateByScESIRF(scTrue,earth, gsTrue,constant,time.obsScType,time);
        % 観測から次ステップまでの推定値と共分散を時間伝搬. 
        [scEstByScEkf.X, scEstByScEkf.P] = Spacecraft.timeUpdateESIRF(scEstByScEkf.X, scEstByScEkf.P, constant, time.scDt2,time.simDt,error);        
    ...
    if isempty(gsTrue.t_dr)
        % 状態量と誤差共分散行列を伝搬
        [scEstByGsEkf.X, scEstByGsEkf.P] = Spacecraft.timeUpdateESIRF(scEstByGsEkf.X, scEstByGsEkf.P, constant, time.simDt,time.simDt,error);
...
% %             time.scDtEstByGs = scTrue.t_dt(1) - error.clock0;
            ...
            [scEstByGsEkf.X_dt, scEstByGsEkf.P_dt] = Spacecraft.timeUpdateESIRF(scEstByGsEkf.X, scEstByGsEkf.P, constant, time.gsDt1, time.simDt,error);        
            ...
            [scEstByGsEkf.X_dt, scEstByGsEkf.P_dt] = Spacecraft.timeUpdateESIRF(scEstByGsEkf.X_dt, scEstByGsEkf.P_dt, constant, time.gsDt1, time.simDt,error);             
        end
        % 観測量の計算
...
%         scEstByGsEkf.observationUpdateByGsEkf(gsTrue,earth,constant,ekf,scTrue,time);
        ...
        [scEstByGsEkf.X, scEstByGsEkf.P] = Spacecraft.timeUpdateESIRF(scEstByGsEkf.X_dt, scEstByGsEkf.P_dt, constant, time.gsDt2, time.simDt,error);
    end
    % 推定値を記録していく
