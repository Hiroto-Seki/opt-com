...
[constant,time,error,gs,sc,gsTrue,earth,scTrue,scEstByScUkf,scEstByGsUkf] = setparam(SSD);
% set parameter related to UKF
ukf = setUkfParam();
...
% 真値の計算
scTrue.calcOrbitTwoBody(error.dynamics)
%% 3. calculate initial guess of spacecraft orbit (without observation)
scEstByScUkf.calcOrbitTwoBody(0)
scEstByGsUkf.calcOrbitTwoBody(0)
...
for i = 1:length(time.list)-1
    ...
    if  i == 1  || i == time.lastSearch + time.obsStep
        ...
        [gsTrue.opnEstTempT_ut,gsTrue.opnEstTempState_ut] ...
            = GroundStation.calcTarget(time.list(i),gsTrue.state(:,i),earth.state(:,i),scEstByGsUkf,time,constant);
        % 真の軌道から，本来の向けるべき方向を計算する
        [gsTrue.opnTrueTempT_ut,gsTrue.opnTrueTempState_ut] ...
            = GroundStation.calcTarget(time.list(i),gsTrue.state(:,i),earth.state(:,i),scTrue,time,constant);
    ...
    if time.list(i+1) < scTrue.t_ur(scTrue.ur_counter + 1) % 次の時刻まで観測がなかった場合
        % 状態量と誤差共分散行列を伝搬
        [scEstByScUkf.X, scEstByScUkf.P, scEstByScUkf.x_sp] = Spacecraft.calcTimeUpdateUkf(scEstByScUkf.x_sp,scEstByScUkf.mu, ukf, time.simDt, time.simDt, error);
    else % 観測があった時
        % 何度目の観測か
...
%             time.obsScType = 1;
        ...
        % 観測までは推定値と共分散を時間伝搬
        [scEstByScUkf.X, scEstByScUkf.P,~] = Spacecraft.calcTimeUpdateUkf(scEstByScUkf.x_sp,scEstByScUkf.mu, ukf, time.scDt1, time.simDt, error);
        % 観測量の計算
        ...
        scEstByScUkf.observationUpdateByScUkf(scTrue,earth, gsTrue,constant,time.obsScType,ukf)
        % シグマポイントを再計算 
        scEstByScUkf.x_sp = Spacecraft.calcSigmaPoint(scEstByScUkf.X, scEstByScUkf.P,ukf);
        % 観測から次ステップまでの推定値と共分散を時間伝搬. 
        [scEstByScUkf.X, scEstByScUkf.P,~] = Spacecraft.calcTimeUpdateUkf(scEstByScUkf.x_sp,scEstByScUkf.mu, ukf, time.scDt2, time.simDt, error);
    ...
    if time.list(i+1) < scTrue.t_ur(1)
        % 状態量と誤差共分散行列を伝搬
        [scEstByGsUkf.X, scEstByGsUkf.P, scEstByGsUkf.x_sp] = Spacecraft.calcTimeUpdateUkf(scEstByGsUkf.x_sp,scEstByGsUkf.mu, ukf, time.simDt, time.simDt, error);
    % 次の時刻まで観測がなかった時
    ...
    % 次の時刻までに観測があった時．
        ...
        if gsTrue.dr_counter == 1 % 初めての観測の時. 実際に送信した時刻は推定値からクロックのオフセットの分だけずれている
            ...
            [scEstByGsUkf.X_dt, scEstByGsUkf.P_dt, scEstByGsUkf.x_sp_dt] = Spacecraft.calcTimeUpdateUkf(scEstByGsUkf.x_sp,scEstByGsUkf.mu, ukf, time.gsDt1, time.simDt, error);      
            ...
            [scEstByGsUkf.X_dt, scEstByGsUkf.P_dt, scEstByGsUkf.x_sp_dt] = Spacecraft.calcTimeUpdateUkf(scEstByGsUkf.x_sp_dt,scEstByGsUkf.mu, ukf, time.gsDt1, time.simDt, error);                 
        end
        % 観測量の計算
        ...
        [scEstByGsUkf.X, scEstByGsUkf.P, scEstByGsUkf.x_sp] = Spacecraft.calcTimeUpdateUkf(scEstByGsUkf.x_sp_dt,scEstByGsUkf.mu, ukf, time.gsDt2, time.simDt, error);             
    end
    % 推定値を記録していく
