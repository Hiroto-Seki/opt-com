...
function [X_bar, P_bar] = updateState(X_hat,P, dt,mu)
   ...
   k2stm = Spacecraft.delFdelX(xvsc+0.5*dt*k1sc,mu)*(STM+0.5*dt*k1stm);
   k3stm = Spacecraft.delFdelX(xvsc+0.5*dt*k2sc,mu)*(STM+0.5*dt*k2stm);
   k4stm = Spacecraft.delFdelX(xvsc+dt*k3sc,mu)*(STM+dt*k3stm);
